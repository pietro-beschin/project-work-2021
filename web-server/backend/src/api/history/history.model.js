...
module.exports.store = async (data) => {            //memorizza dati su mongodb
    ...
    data.data_aggiornamento = moment(new Date()).utcOffset(-2, true);                   //crea data aggiornamento = adesso
    if(result = await historySchema.findOne({codice_commessa : data.codice_commessa})){ //se esiste già e codice commessa c'è
        return await historySchema.findByIdAndUpdate(result._id, data);                         //la aggiorno
    }else if(ultimaInserita = await historySchema.findOne().sort({'_id' : -1})){        //altrimenti se c'è almeno una commessa
        let stato = (ultimaInserita.quantita_prodotta >= ultimaInserita.quantita_prevista) ? "completata" : "fallita";  //calcola stato della commessa precedente
        await historySchema.findByIdAndUpdate(ultimaInserita._id, {"stato" : stato}, {"data_aggiornamento" : data.data_aggiornamento});  //assegna stato a quella commessa
    }

    if(data.codice_commessa != ""){         //se il codice non è vuoto   
        data.stato = 'in esecuzione';
        console.log("NUOVO");                   
        return await historySchema.create(data); //salvo la nuova commessa con stato 'in esecuzione'    
    }
}
