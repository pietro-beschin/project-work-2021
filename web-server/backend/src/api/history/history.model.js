...
module.exports.list = async (query) => {
        ...
        if(query.from){
            q.data_esecuzione.$gte = moment(new Date(query.from)/*.setHours(0,0,0,000))*/.utcOffset(0, true));
        }
        if(query.to){
            q.data_esecuzione.$lte = moment(new Date(query.to)/*.setHours(23,59,59,999))*/.utcOffset(0, true));
        }
